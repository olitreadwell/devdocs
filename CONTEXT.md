...
- `2026-09-24` — repo-audit (tests/CI + clean-code): `Searcher.normalizeString` / `Searcher.normalizeQuery` (core search normalisation: collapses separators, strips decoration) have no direct unit tests — only indirect coverage through `search_ranking_test`. Real and verified on upstream main @fde566c2: both static methods are pure and their rules are subtle (C#/C++ separators, `(class)` and `()` stripping, the `event$` suffix, EOS separators), and no test calls them directly. Dedupe: `normalizeString` / `search normalization` searches find no upstream issue/PR. Fixed by adding `test/assets/search_normalization_test.js` (5 tests). Outcome: pr-opened (fork #11).

- `2026-09-25` — trivial/minor pass (loop-trivial). Verified current upstream main @ fde566c2 (fork main synced, #2725+#2741 merged). Exhaustive search of editable content (README, docs/*, .github templates, lib/comments, views/assets, codespell + manual misspell net + full external-link curl + internal path/anchor/thor-command validation): repo prose is clean after prior passes. Found exactly TWO genuine dead links, both in `docs/file-scrapers.md` (below the loop's 3-fix minimum, so held, no PR — outcome skipped): (1) gnuplot `git clone https://git.code.sf.net/p/gnuplot/gnuplot-main` (SourceForge retired git.code.sf.net; verified 404) → live cloneable `https://github.com/gnuplot/gnuplot` (verified 200, ls-remote works; official mirror); (2) OpenJDK example `openjdk-25-doc_25+36-1_all.deb` (verified 404) → current `openjdk-25-doc_25.0.4.1+1-1_all.deb` (verified 200; curl + tar xf lines). No other genuine, meaning-preserving trivial errors found. Fewer than 3 genuine fixes → SKIP per trivial_fix_rules (never pad). status: skipped (no PR).
...
- `2026-09-24` — core search normalisation (`Searcher.normalizeString`/`normalizeQuery`) had no direct unit tests. status: attempted → pr-opened (fork #11).

- `2026-09-25` — trivial doc cleanup PR (fork #12): grammar `and then outputs the result → and then output the result` (filter-reference.md, subject-verb agreement); `a few hundreds pages → a few hundred pages` (adding-docs.md); `Verify this steps → Verify these steps` (bug_report.md + feature_request.md, issue-template comments); README `test:coverage` description corrected from "App tests" to the task's own "coverage report for core tests". Codespell across lib/, views/, assets/, .github/, docs/, README returned zero genuine typos (all hits were false positives: statics/vertx/sugget/smove/minite/ment/als substrings); no dead external links in the docs (all markeddown links returned 200). Outcome: pr-opened (5 files, <=10 cap, >=3 fixes).
